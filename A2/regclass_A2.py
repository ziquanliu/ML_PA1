...
def Bayes_Pred(Mat_Feat,theta_post,round=0):
    ...
    if round==1:
        Bay_pred = {'miu_pred': np.round(miu_pred), 'sgm_pred': sgm_pred}
    else:
        Bay_pred={'miu_pred':miu_pred,'sgm_pred':sgm_pred}
    return Bay_pred


def nonBayes_Plt(smpx,smpy,smp_pre_y,polyx,polyy,poly_pre_y,name):
    f = plt.figure(name)
...
def Bayes_Ply(smpx,smpy,Train_Pred,polyx,polyy,Test_Pred):
    plt.plot(polyx.transpose(), polyy, '-', color='k', label='True Poly')
    plt.errorbar(polyx.transpose(), Test_Pred['miu_pred'], yerr=Test_Pred['sgm_pred'], fmt='.',
                 ecolor='blue', color='blue', label='Estimated Poly')
    plt.plot(smpx.transpose(), smpy, '.', color='green', label='True Sample')
    plt.errorbar(smpx.transpose(), Train_Pred['miu_pred'], yerr=Train_Pred['sgm_pred'], fmt='.',
                 ecolor='red', color='red', label='Estimated Sample')
    plt.legend()
    plt.title('Bayes Regression')
    plt.savefig('Bayes Regression', dpi=150)
    plt.show()
...
def Pred_All_Err_Round(truey,Test_Pred,Bayes_Test_Pred):
    Pred_Error = {}
    n_train = truey.shape[0]
    y=truey.copy()
    y=np.around(y)
    ...
    for key in Test_Pred:
        error_sq = np.square(np.reshape(Test_Pred[key], (n_train, 1)) - y)
        error = np.sum(error_sq) / n_train
        Pred_Error[key] = error

    Bayes_err_sq = np.square(Bayes_Test_Pred['miu_pred'] - y)
    Pred_Error['Bayes'] = np.sum(Bayes_err_sq) / n_train
    return Pred_Error

def Pred_Abs_Err(truey,Test_Pred,Bayes_Test_Pred):
    Pred_Error = {}
    n_train = truey.shape[0]
    y=truey.copy()
    y=np.around(y)
    for key in Test_Pred:
        error_ab = np.absolute(np.reshape(Test_Pred[key], (n_train, 1)) - y)
        error = np.sum(error_ab) / n_train
        Pred_Error[key] = error
    Bayes_err_ab = np.absolute(Bayes_Test_Pred['miu_pred'] - y)
    Pred_Error['Bayes'] = np.sum(Bayes_err_ab) / n_train
    return Pred_Error
...
def Task_b(Fea_Trn,smpx,smpy,Fea_Tst,polyx,polyy,var,hyperp,org_samy):
    ...
    for key in theta:
        nonBayes_Plt(smpx, org_samy, Train_Pred[key], polyx, polyy, Test_Pred[key], key)

    # Bayes
    Bayes_Ply(smpx, org_samy, Bayes_Train_Pred, polyx, polyy, Bayes_Test_Pred)
...
def Counting(trainx,trainy,testx,testy,variance,hyperp):
    regress = Regressor(trainx, trainy, variance)
    rgl_lambda = hyperp['rgl']  # hyperparameter of regulized LS
    lasso_t = hyperp['lasso']  # hyperparameter of LASSO
    Bay_alpha = hyperp['Bayes']  # hyperparameter of Bayes
    theta = {'LSE': regress.LSE_Reg(), 'Rgl': regress.Regul_Reg(rgl_lambda), 'Las': regress.LASSO_Reg(lasso_t),
    ...
    Test_Pred = {}
    for key in theta:
        Test_Pred[key] = np.round(nonBayes_Pred(theta[key], testx))
    Bayes_Test_Pred = Bayes_Pred(testx, Bayes_theta, 1)
    Pred_Ab_Err = Pred_Abs_Err(testy, Test_Pred, Bayes_Test_Pred)
    Pred_Sq_Err = Pred_All_Err_Round(testy, Test_Pred, Bayes_Test_Pred)
    print 'Abs', Pred_Ab_Err
    print 'Sq', Pred_Sq_Err
    for key in theta:
        f = plt.figure()
        plt.plot(range(testy.shape[0]), testy, '-.', color='k', label='True Number')
        plt.plot(range(testy.shape[0]), Test_Pred[key], '-.', color='r', label='Predict Number')
        plt.legend()
        plt.title(key)
        plt.xlabel('MSE='+str(Pred_Sq_Err[key])+', MAE='+str(Pred_Ab_Err[key]))
        plt.ylabel('num of people')
        plt.savefig(key+'.eps', dpi=400)
        #plt.show()
    fig = plt.figure('Bayes')
    plt.plot(range(testy.shape[0]), testy, '-.', color='k', label='True Number')
    plt.errorbar(range(testy.shape[0]), Bayes_Test_Pred['miu_pred'], yerr=Bayes_Test_Pred['sgm_pred'], fmt='-.',
                 ecolor='blue', color='r', label='Predict Number')
    plt.legend()
    plt.title('Bayes Regression')
    plt.xlabel('MSE='+str(Pred_Sq_Err['Bayes'])+', MAE='+str(Pred_Ab_Err['Bayes']))
    plt.ylabel('num of people')
    plt.savefig('BayesRegression.eps', dpi=400)
    #plt.show()
    Pred_Error={'AbsErr':Pred_Ab_Err,'SquErr':Pred_Sq_Err}
    return Pred_Error
